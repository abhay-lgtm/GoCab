...
export default function VoiceBooking() {
  ...
  const inputStyle = {
    width: '100%', height: 40, borderRadius: 4, fontSize: 14, padding: '0 12px',
    background: '#111827',
    ...
    <div className="flex flex-col w-full max-w-xl mx-auto mt-6 gap-8">
      <div className="text-center mb-10">
        <div
          ...
          style={{ background: '#111827', border: '1px solid rgba(167,139,250,0.25)', color: '#8b5cf6' }}
        >
          <Mic size={12} />
          Voice Booking
        </div>
      ...
      {(state === 'idle' || state === 'listening') && (
        <div className="flex flex-col items-center gap-8">
          ...
          <div className="relative">
            {state === 'listening' && (
              <>
                <div className="absolute inset-0 rounded-full animate-ping scale-150" style={{ background: '#111827' }} />
                <div className="absolute inset-0 rounded-full scale-125" style={{ background: '#111827' }} />
              </>
            )}
            <button
          ...
          {state === 'listening' && (
            <div className="animate-fade-in">
              <VoicePulse active />
              {transcript && (
                <p className="text-center text-sm italic mt-4" style={{ color: '#9ca3af' }}>
      ...
      {state === 'recognized' && (
        <div className="flex flex-col gap-4 animate-slide-up">
          <div
            className="rounded p-4 flex items-center gap-3 mb-2"
          ...
          {!editing ? (
            <div
              className="flex flex-col rounded p-5 gap-4"
              style={{ background: '#111827', border: '1px solid #1f2937' }}
            >
              <div>
                <p className="text-xs font-medium uppercase tracking-wide mb-1" style={{ color: '#6b7280' }}>Pickup</p>
                <p className="text-base font-semibold" style={{ color: '#f9fafb' }}>{recognizedData?.pickup}</p>
              </div>
              <div style={{ borderTop: '1px solid #1f2937', paddingTop: 16 }}>
                <p className="text-xs font-medium uppercase tracking-wide mb-1" style={{ color: '#6b7280' }}>Destination</p>
                <p className="text-base font-semibold" style={{ color: '#f9fafb' }}>{recognizedData?.destination}</p>
              </div>
            </div>
          ) : (
            <div
              className="flex flex-col rounded p-5 gap-3"
              style={{ background: '#111827', border: '1px solid #1f2937' }}
            >
              <div>
                <label className="text-xs font-medium uppercase tracking-wide" style={{ color: '#6b7280' }}>Pickup</label>
                <input style={inputStyle} value={pickup} onChange={e => setPickup(e.target.value)} />
              </div>
              <div>
                <label className="text-xs font-medium uppercase tracking-wide" style={{ color: '#6b7280' }}>Destination</label>
